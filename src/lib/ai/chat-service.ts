...
async function captureRecommendationFromToolCall(args: {
  ...
  switch (args.toolName) {
    ...
    case "setAdSetBudget":
      ...
      targetMetaId = String(args.toolArgs.adSetId ?? args.toolArgs.adsetId ?? "");
      break;
    case "schedulePagePost":
      actionType = "refresh_creative";
      // Organic post — closest existing target kind is "ad" (the post is
      // a content asset). When/if we add a "page_post" target kind we
      // should switch this; for now "ad" preserves capture continuity.
      targetKind = "ad";
      targetMetaId = String(args.toolArgs.pageId ?? "");
      break;
    default:
...
- Use schedulePagePost to compose + schedule an ORGANIC Facebook page post — NOT a paid ad. Pair with the "creative refresh" recommendation when the user is willing to publish a new post (rather than just pause and rebuild ads). The user must supply the page id (from /t/[slug]/settings) and uploaded media URLs (from /api/posts/upload) — never invent these.
