...
async function main() {
  ...
  // Manual test script defaults to Thai locale (matches founder + early-customer base).
  const LOCALE = "th" as const;

  // 1. Verify email (ที่ user ได้หลัง signup)
  console.log("1/3: Verify email...");
  const verifyTpl = await verifyEmailTemplate({
    name: USER_NAME,
  ...
  const reminderTpl = await trialReminder2dTemplate({
    name: USER_NAME,
    planName: "Pro",
    priceThb: 10990,
  ...
  const receiptTpl = await invoicePaidTemplate({
    name: USER_NAME,
    amountThb: 10990,
    baseThb: 10271,
    vatThb: 719,
